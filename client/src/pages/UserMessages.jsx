...
const UserMessages = () => {
    ...
    useEffect(() => {
        ...
        socket.on('new_message', (data) => {
            fetchMessages(); // refresh seamlessly
        });
    ...
    const groupConversations = (allMessages) => {
        ...
        // Auto-select Admin if they are the only contact and not selected yet
        if (conversationList.length > 0 && !activeConversation) {
            setActiveConversation(conversationList[0].partner._id);
    ...
    // If perfectly blank and admin is fetched, show admin as a contact option
    const renderConversations = () => {
        let displayList = [...conversations];

        // If user has NO messages yet, artificially inject the Admin so they can start a chat!
        if (displayList.length === 0 && adminContact) {
            displayList.push({
                partner: adminContact,
                messages: [],
    ...
    const activeConvData = conversations.find(c => c.partner._id === activeConversation) ||
        (renderConversations().find(c => c.partner._id === activeConversation));
    ...
    return (
        <DashboardLayout role="user" user={user}>
            ...
            <div className="flex h-[calc(100vh-140px)] bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                ...
                <div className={`${activeConversation ? 'hidden md:flex' : 'flex'} w-full md:w-1/3 lg:w-1/4 border-r border-gray-200 flex-col bg-gray-50 flex-shrink-0 z-10`}>
                    <div className="p-4 border-b border-gray-200 bg-white font-bold text-gray-700 uppercase text-xs tracking-wider flex justify-between items-center">
                        <span>Contacts</span>
                        <span className="bg-zegen-blue text-white rounded-full px-2 py-0.5 text-[10px]">{renderConversations().length}</span>
                    </div>
                            ...
                            >
                                {conv.partner.profileImage || conv.partner.avatar ? (
                                    <img
                                        ...
                                        className="w-10 h-10 rounded-full object-cover shadow-sm border border-gray-200 flex-shrink-0"
                                    />
                                ) : (
                                ...
                                <div className="flex-grow min-w-0">
                                    ...
                                    <p className={`text-xs truncate ${conv.unreadCount > 0 ? 'font-bold text-zegen-blue' : 'text-gray-500'}`}>
                                        {conv.isNew ? 'Click to start chatting...' : conv.messages[conv.messages.length - 1]?.content}
                                    </p>
                                </div>
                                {conv.unreadCount > 0 && (
                ...
                <div className={`${!activeConversation ? 'hidden md:flex' : 'flex'} w-full md:w-2/3 lg:w-3/4 flex-col bg-[#f8fbff] relative`}>
                    {activeConvData ? (
                        <>
                            {/* Chat Header */}
                            <div className="p-3 border-b border-gray-200 bg-white flex items-center justify-between shadow-sm z-10 sticky top-0">
                                <div className="flex items-center gap-3">
                                    <button
                                        className="md:hidden mr-2 text-gray-500 hover:text-zegen-blue p-2 rounded-full hover:bg-gray-100 transition-colors"
                            ...
                            <div className="flex-grow p-4 md:p-6 overflow-y-auto flex flex-col gap-3 scrollbar-thin scrollbar-thumb-gray-300">
                                {activeConvData.messages.length === 0 && (
                                    <div className="bg-yellow-50 text-yellow-800 text-xs text-center p-2 rounded-lg shadow-sm mx-auto my-4 max-w-sm border border-yellow-200">
                                        Messages are end-to-end secured. Start a conversation with the Administrator to get help or guidance safely.
                                    </div>
                                )}
                                ...
                                {activeConvData.messages.map(msg => (
                                    ...
                                    >
                                        ...
                                        <div className={`absolute top-0 -mt-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-20 ${msg.type === 'sent' ? '-left-16' : '-right-10'}`}>
                                            ...
                                            {/* Users likely shouldn't hard-delete messages from thread arbitrarily unless required, but let's leave it per previous spec */}
                                            <button
